...
interface VideoCardProps {
  ...
  onChangeVideoCategory: (videoId: string, newCategory: string) => void;
}
...
export default function VideoCard({
  ...
  const handleCategoryChange = (category: string) => {
    onChangeVideoCategory(video.id, category);
    setShowMenu(false);
    setShowCategorySubmenu(false);
  };
  ...
  const handleCreateCategory = (e: React.FormEvent) => {
    ...
    if (newCategoryName.trim()) {
      onChangeVideoCategory(video.id, newCategoryName.trim());
      setNewCategoryName("");
      setShowMenu(false);
      setShowCategorySubmenu(false);
    }
            ...
            <div ref={menuRef} className="relative flex-shrink-0">
              ...
              {showMenu && (
                ...
                >
                  {/* Favorites */}
                  <button
                    onClick={() => { onToggleFavorite(video.id); setShowMenu(false); }}
                  ...
                  {/* Add to Course */}
                  <button
                    onClick={() => { setShowPlaylistSubmenu(!showPlaylistSubmenu); setShowCategorySubmenu(false); }}
                  ...
                  {showPlaylistSubmenu && (
                    <div className="px-3 py-2 bg-neutral-50/50 dark:bg-neutral-900/40 border-y border-border-custom space-y-2 max-h-40 overflow-y-auto no-scrollbar">
                      {courses.map((course) => {
                        const alreadyIn = course.videoIds.includes(video.id);
                        return (
                          <button
                            key={course.id}
                  ...
                  {/* Change Category */}
                  <button
                    onClick={() => { setShowCategorySubmenu(!showCategorySubmenu); setShowPlaylistSubmenu(false); }}
                    className="w-full px-4 py-2 hover:bg-neutral-100/70 dark:hover:bg-neutral-800/70 flex items-center text-left cursor-pointer transition-colors justify-between text-text-main"
                  >
                    <span className="flex items-center">
                  ...
                  {showCategorySubmenu && (
                    <div className="px-3 py-2 bg-neutral-50/50 dark:bg-neutral-900/40 border-y border-border-custom space-y-1 max-h-40 overflow-y-auto no-scrollbar">
                      {defaultCategories.map((cat) => (
                        ...
                        >
                          <span>{cat}</span>
                          {video.category === cat && <Check className="w-3.5 h-3.5 text-green-600" />}
                        </button>
                      ))}
                      <form onSubmit={handleCreateCategory} className="flex items-center space-x-1 pt-1.5 border-t border-border-custom">
                        <input
                          ...
                          className="w-full px-2 py-0.5 text-[10px] border border-border-custom rounded focus:outline-none focus:border-red-500 bg-bg-card text-text-main"
                        />
                        <button type="submit" className="p-1 bg-red-600 hover:bg-red-700 text-white rounded cursor-pointer" title="Create & Move">
                          <Plus className="w-3 h-3" />
                        </button>
                      </form>
                    </div>
                  ...
                  {/* Edit */}
                  <button
                    onClick={() => { onEditVideo(video); setShowMenu(false); }}
