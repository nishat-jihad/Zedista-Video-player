...
interface VideoCardProps {
  ...
  onUpdateVideoCategory?: (videoId: string, newCategory: string) => void;
...
export default function VideoCard({
  ...
  const handleCategorySelect = (cat: string) => {
    if (onUpdateVideoCategory) {
      onUpdateVideoCategory(video.id, cat);
    }
    setShowMenu(false);
    setShowCategorySubmenu(false);
  };

  const handleCreateAndSetCategory = (e: React.FormEvent) => {
    e.preventDefault();
    ...
    >
      ...
      <div className="pt-3 pb-1 flex gap-3 flex-1 w-full">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex justify-between items-start gap-1">
            ...
            <div ref={menuRef} className="relative flex-shrink-0">
              ...
              >
                ...
                <button
                  id={`video-menu-category-${video.id}`}
                  onClick={() => {
                ...
                {showCategorySubmenu && (
                  <div className="px-3 py-2 bg-neutral-50/50 dark:bg-neutral-900/40 border-y border-border-custom space-y-2 max-h-40 overflow-y-auto no-scrollbar">
                    {allCategories
                      ...
                      .map((cat) => {
                        const isCurrent = video.category === cat;
                        return (
                          <button
                            key={cat}
                            id={`video-menu-set-category-${cat}-${video.id}`}
                    ...
                    {/* Form to create & assign new category */}
                    <form onSubmit={handleCreateAndSetCategory} className="flex items-center space-x-1 pt-1.5 border-t border-border-custom">
                      <input
                        type="text"
          ...
          <p className="text-xs text-text-sub mt-1 truncate" title={video.channelName}>
